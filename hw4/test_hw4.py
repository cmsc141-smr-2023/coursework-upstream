...
def test_how_many_groups_fit(vals, target, expected):
    """
    Do a single test for Exercise 1: how_many_groups_fit
    """
    recreate_msg = helpers.gen_recreate_msg(MODULE, "how_many_groups_fit", vals, target)
    vals_copy = vals[:]
    actual = hw4.how_many_groups_fit(vals_copy, target)
...
def test_gen_fans(names, sizes, expected):
    """
    Do a single test for Exercise 2: gen_fans
    """
    
    recreate_msg = helpers.gen_recreate_msg(MODULE, "gen_fans", names, sizes)

    names_copy = names[:]     
    sizes_copy = sizes[:]

    actual = hw4.gen_fans(names_copy, sizes_copy)
...
def check_section(actual, expected, recreate_msg, extra_str=""):
    """
    Compare an actual and expected section data structure.
    """
    ...
    msg = (f"\n\n{extra_str}The function returned a value for the section of the wrong type.\n"
           f"  Expected type of the section: list[str] \n")
    actual_type = type(actual)
    msg2 = f"  Actual type of the section: {actual_type.__name__}\n"

    assert isinstance(actual, list), msg + msg2 + recreate_msg

    helpers.check_length(actual, expected, recreate_msg, "of the section")
...
@pytest.mark.timeout(60)
@pytest.mark.parametrize("names, sizes, section_config, expected", fill_section_tests)
def test_fill_section(names, sizes, section_config, expected):
    """
    Do a single test for Exercise 4: fill_section
    """
    
    recreate_msg = helpers.gen_recreate_msg(MODULE, "fill_section", names, sizes, section_config)

    names_copy = names[:]
    sizes_copy = sizes[:]

    actual = hw4.fill_section(names_copy, sizes_copy, section_config)

    # verify that the function did not modify the list inputs
    helpers.check_list_unmodified("names", names, names_copy, recreate_msg)
    helpers.check_list_unmodified("sizes", sizes, sizes_copy, recreate_msg)    
    
    check_section(actual, expected, recreate_msg)
     ...
     (["Mets","Cubs", "Giants", 'Yankees', 'Dodgers', 'Phillies', 'Orioles'],
      ...
      [['Mets-1,Mets-2',
       ...
       ['Phillies-1,Phillies-2',
        ...
        'Empty,Empty',
        'Empty,Empty',
        'Empty,Empty']])] + long_tests["stadium"]
      



@pytest.mark.timeout(60)
@pytest.mark.parametrize("names, sizes, section_config, expected", fill_stadium_tests)
def test_fill_stadium(names, sizes, section_config, expected):
    """
    Do a single test for exercise 5: fill_stadium
    """
    recreate_msg = helpers.gen_recreate_msg(MODULE, "fill_stadium",
                                            names, sizes, section_config)

    names_copy = names[:]
    sizes_copy = sizes[:]

    actual = hw4.fill_stadium(names_copy, sizes_copy, section_config)
    ...
    helpers.check_length(actual, expected, recreate_msg, "sectiones")
    ...
    for i, (actual_val, expected_val) in enumerate(zip(actual, expected)):
        ...
        check_section(actual_val, expected_val, recreate_msg,
                  f"Checking element {i} in the list of sectiones")
